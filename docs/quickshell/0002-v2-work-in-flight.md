...
- [herdr/default.nix](../../modules/apps/agent/herdr/default.nix) already manages the package, TOML configuration, and declared plugin links.
- [default.nix](../../modules/apps/agent/default.nix) composes herdr, enables the existing reviewr integration, and selects system toast delivery. Keep that workflow intact.
...
- `Theme.barGlass(occupied)` — one backing used by both bars only: 0.68 while the workspace holds a window, and 0.20 while it holds none, so a bright or light wallpaper no longer dominates a bar with content to sit next to and an empty workspace keeps most of the prototype's open look without giving up the text. Text stays fully opaque either way.
