...
- If an input has a binary cache dedicated to it, such as `llm-agents.nix`, add no follows at all for that input. Following shared roots changes its dependency graph and derivation hashes, which misses its cache. Leave its inputs as declared upstream.
...
The nixpkgs exception is task-specific: do not preserve an old exception after the user asks to return that input to the default policy. The `flake-parts` and `systems` follows have no policy exception, except for a cache-backed input covered above; if an upstream input exposes either dependency, follow the shared root.

## Workflow

1. Scan every `flake-file.inputs` declaration and inspect `flake.lock` as a dependency graph. Identify parents with `nixpkgs`, `flake-parts`, or `systems` edges, including suffixed duplicate nodes such as `flake-parts_2` or `systems_2`. Skip any input backed by a dedicated binary cache.
2. Add follows beside the feature-owned `flake-file.inputs.<name>` declaration, except for cache-backed inputs, which get none. Preserve intentionally different nixpkgs policy only when explicitly requested. Do not edit generated `flake.nix` directly.
